...
exports.checkAuthorization = (req, res, userType) => {
    ...
    if(userType == null) return res.status(500).send('`user_type` parameter is required');
    ...
    if(!validUserType) return res.status(500).send('The value for `user_type` parameter is invalid');
